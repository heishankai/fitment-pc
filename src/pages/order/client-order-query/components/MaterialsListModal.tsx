...
const MaterialsListModal = ({ tableFormRef }: any, ref: any) => {
  ...
  // 辅材清单支付
  const handleMaterialsPay = async (index: number) => {
    const { id } = record ?? {};

    const { success } = await payService({
      order_id: id,
      pay_type: 'materials_list',
      subItem: index,
    });
  ...
  const materialsList = record?.materials_list || [];
                  ...
                  render: (_: any, entity: any) => {
                    ...
                    return (
                      <Popconfirm
                        ...
                        onConfirm={() =>
                          handleMaterialsPay(entity?.__materialsListIndex)
                        }
                      >
