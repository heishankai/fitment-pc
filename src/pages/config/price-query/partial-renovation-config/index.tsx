...
// components
import PartHouseModal from './components/PartHouseModal';
// utils
import { getProTableConfig } from '@/utils/proTable';
// service
import {
  getPartialRenovationConfigListService,
  deletePartialRenovationConfigService,
} from './service';

const Table = () => {
  const partHouseModalRef = useRef<any>(null);
  const actionRef = useRef<ActionType>();
  const tableFormRef = useRef<ProFormInstance>();

  // 删除案例
  const handleDelete = async (id: string | number) => {
    const { success } = await deletePartialRenovationConfigService(id);
    if (success) {
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        {...getProTableConfig({
          request: async (params) => {
            return await getPartialRenovationConfigListService(params);
          },
        })}
        rowKey="id"
        scroll={{ x: 1200 }}
        headerTitle={
          <Space>
            <Button
              icon={<PlusOutlined />}
              type="primary"
              onClick={() => partHouseModalRef.current.handleOpenModal('add')}
            >
              新增全屋装修配置
            </Button>
          </Space>
              ...
              return (
                <Space>
                  <Button
                    ...
                    icon={<EditOutlined />}
                    onClick={() =>
                      partHouseModalRef.current.handleOpenModal('edit', record)
                    }
                  >
                    编辑
                  </Button>
      ...
      />
      <PartHouseModal ref={partHouseModalRef} tableFormRef={tableFormRef} />
    </PageContainer>
  );
};
