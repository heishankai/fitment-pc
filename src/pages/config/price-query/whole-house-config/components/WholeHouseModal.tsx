...
import {
  createWholeHouseConfigService,
  editWholeHouseConfigService,
} from '../service';

const { useWatch } = Form;

const WholeHouseModal = (props: any, ref: any) => {
  ...
  const handleFinish = async (values: any) => {
    const wholeHouseConfigService =
      title === 'add'
        ? createWholeHouseConfigService
        : editWholeHouseConfigService;

    const { success } = await wholeHouseConfigService(
      record?.id,
          ...
          <Col span={12}>
            <ProFormText
              ...
              fieldProps={{
                maxLength: 50,
                showCount: true,
                placeholder: '如：全屋装修工长服务（购房面积50平方米内）',
              }}
            />
          </Col>
          <Col span={12}>
...
export default forwardRef(WholeHouseModal);
